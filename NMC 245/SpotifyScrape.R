...
timevalues <- seq(as.Date("2017/01/01"), as.Date("2017/12/31"), by = "day")
...
saveRDS(spotify, "/Users/dunk/Classes/NMC 245/spotifyRAW.rds")


spotify <- readRDS("/Users/dunk/Classes/NMC 245/spotifyRAW.rds")
dim(spotify)
...
by_streams %>%
  ...
  labs(title = 'US 2017 | Most Streamed Songs',
       x = "Track Name",
       y = "Total Streams") + 
...
by_artist %>%
  ...
  coord_flip() +
  geom_segment(aes(y = 334342280, x = 20, xend = 20, yend = 334342281), arrow = arrow(length = unit(0.5, "cm")), color = "darkblue") +
  annotate(geom = "text", x = 20, y = 125000000, label = "Streams from HUMBLE.") +
  labs(title = 'US 2017 | Most streamed Artist',
       x = "Artist Name",
       y = "Total Streams") + 
  theme(plot.title = element_text(hjust = 0.5),
        axis.text.x = element_text(vjust = 0.6),
...
ggplot(data=by_WeekDay, aes(x=WeekDay, y=TotalStreams, group=1)) +
  ...
  geom_point() +
  scale_y_continuous(labels = scales::comma) +
  labs(title = 'US 2017 | Most Streamed Day of the Week | Friday Reached ~ 4 Billion', #3,928,599,112
       x = "Day of the Week",
...
ggplot(data=by_Month, aes(x=Month, y=TotalStreams, group=1)) +
  ...
  geom_point()+
  scale_y_continuous(labels = scales::comma) +
  labs(title = 'US 2017 | Most streamed Month | February Reached ~2.4 Billion', # 2,381,851,473
       x = "Month",
       y = "Total Streams")
...
saveRDS(top100songs, file = "/Users/dunk/Classes/NMC 245/top100songs.rds")
